...
public class ChattingServer {
	...
	public void message(Session session, String msg) { // "발송자, 수신자, 내용"
		...
		//0번인덱스 : 발송자 , 1번인덱스는 : 수신자, 2번인덱스 : 메세지
		
		//보낸데이터 세션에 저장할수 있음
		session.getUserProperties().put("msg", data);
		//클라이언트가 보낸 데이터를 다른 접속한 클라이언트에게 전송하기;
		// 1. 접속한 클라이언트 관리
		// 1) Conllection클래스를 통해 Map, List, Set을 이용해서 관리
		// 2) session클래스에서 접속한 모든 session을 알수있는 메소드를 사용 -> 버전2 에서 사용할 예정
		
		// 2. 접속한 session객체를 이용해서 데이터를 전송할수 있음
		// 	  session.getBasicRemot() -> 접속한 세션과 연결되는 스트림을 가져옴
