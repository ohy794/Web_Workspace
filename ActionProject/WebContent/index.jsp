...
<body>
	...
	<!-- 
		* JSP를 이루는 구성인자
		...
		1. JSP 스크립팅원소 : JSP페이지 안에서 자바코드를 직접 기술할게 있는 기술
			ex) 선언문, 스크립틀릿, 표현식, 지시어...
		
		2. 지시어 : JSP페이지 정보에 대한 내용을 표현한다거나 또 다른 페이지를 포함할때 사용
			ex) 지시어 (page 지시어 , include지시어, taglib 지시어(라이브러리 추가시 사용)
		...
		3. JSP 액션태그 : XML기술을 이용해서 기존 JSP문법을 확장하는 기술을 제공하는 태그
				...
				- 커스텀액션태그(Custom Action Tag) : JSP페이지에서 바로 사용불가능(별도의 라이브러리 연동해줘야함)
													커스텀액션태그는 모든 태그명 안에 별도의 접두어 붙음(c:, fn:, fmt:)
													제공되고 있는 대표적인 유용한 라이브러리가(JSTL)
	 -->
	
	<h1>*JSP Action tage</h1>
	<p>
		XML기술을 기용해서 기존 JSP문법을 확장시키는 기술을 제공하는 태그들
	</p>
	
	<h3>1. 표준 액션태그</h3>
	<p>
		JSP페이지에서 별도의 라이브러리 연동없이 바로 사용가능함<br>
		태그 앞에 jsp:접두어가 붙음
	</p>
