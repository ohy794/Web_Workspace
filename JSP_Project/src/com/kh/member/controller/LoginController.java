...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * <GET 방식과 POST방식 차이>
		 * - GET 방식 : 사용자가 입력한 값들이 url노출 o/ 데이터 길이제한 O/ 즐겨찾기 기능상 편린
		 * - POST 방식 : 						X/        X     / 대신 Timeout이 존재
		 */

		// 1) 전달값에 한글이 있을경우 인코딩 처리해야함(POST 방식일때만)
		...
		//4) 처리된결과를 가지고 사용자가 보게될 화면(view)를 지정 후 포워딩 or url재요청
		 ...
		 * 					한번 담은 데이터는 내가 직접 지우기 전까지, 서버가 멈추기전까지, 브라우저가 종료되기 전까지 접근해서 쓸수있음
		 * 3) request     : request에 담은 데이터는 해당 request를 포워딩한 응답 jsp에서만 꺼내쓸수있다
		...
		if(loginUser == null) {//로그인 실패 => 에러페이지 응답
			...
			// 포워딩 방식 : 해당 경로로 선택된 뷰가 보여질 뿐 url은 변경되지 않는다.(요청했을때의 url(login.me)이 그대로남아있음)
			view.forward(request, response);
			
		}else {// 로그인 성공 => index페이지(메인페이지) 응답
			
			// 로그인한 회원의 정보를 로그아웃하기 전까지 계속 가져다 쓸것이기 때문에 session에 데이터담기
			...
			// 1. 포워딩 방식 응답뷰 출력하기
			// 선택된 jsp가 보여질뿐 url에는 여전히 이 서블릿에 대한 매핑값이 남아있을것
			// localhost:8082/jspproject/login.me
			// url에 내용이 그대로 남아있는 상태에서 새로고침(f5)누르게되면 계속 로그인 요청을 보내게될것
			
			// 2. url 재요청방식(senRedirect 방식)
